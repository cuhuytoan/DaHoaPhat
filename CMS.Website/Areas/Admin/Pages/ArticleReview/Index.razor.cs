...
namespace CMS.Website.Areas.Admin.Pages.ArticleReview
{
    public partial class Index : IDisposable
    {
        ...
        #region Model
        private List<SpArticleReviewSearchResult> lstArticleReview;
        public int currentPage { get; set; }
        public int totalCount { get; set; }
        public int pageSize { get; set; } = 30;
        public int totalPages => (int)Math.Ceiling(decimal.Divide(totalCount, pageSize));
        public ArticleReviewSearchFilter modelFilter { get; set; }
        ...
        protected async Task InitData()
        {
            ...
            var modelFilter = new ArticleReviewSearchFilter();
            modelFilter.ArticleReviewStatusId = articleRevStatusId;
            modelFilter.Keyword = keyword;
            modelFilter.CurrentPage = p ?? 1;
            modelFilter.PageSize = 30;

            var result = await Repository.ArticleReview.ArticleReviewSearch(modelFilter);
        ...
        protected async Task OnPostDemand(int postType)
        {
            ...
            else
            {
                try
                {
                    foreach (var item in listArticleSelected)
                    {
                        await Repository.ArticleReview.ArticleReviewUpdateStatus(item, postType);
                    }
        ...
        protected void OnCheckBoxChange(bool headerChecked, int ArticleId, object isChecked)
        {
            if (headerChecked)
            {
                if ((bool)isChecked)
                {
                    listArticleSelected.AddRange(lstArticleReview.Select(x => x.Id));
                    isCheck = true;
                }
                else
                {
